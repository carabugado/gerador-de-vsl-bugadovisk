...
import llm
import vsl_context
from transcribe import transcribe, transcribe_composition
import highlights
...
import copy_chief
from compliance import apply_compliance, detect_vertical
...
def _load_keys_into_env():
    ...
    if cfg.get("llm_backend") and cfg["llm_backend"] != "ollama":
        os.environ["LLM_BACKEND"] = cfg["llm_backend"]
...
def llm_status():
    ...
    return {
        ...
        "backends": llm.status(),  # {ollama, gemini, anthropic}
    }
...
_CONFIG_FIELDS = [
    ...
    "gemini_api_key", "gemini_api_keys", "generated_dir", "video_path",
    "llm_backend", "provider_override",
]
...
async def process(req: ProcessRequest):
    ...
    # Modelo escolhido no painel. "ollama" (ou vazio) = cadeia padrão (ollama trabalha,
    # Gemini/Anthropic de apoio). "gemini"/"anthropic" = força aquele modelo nas etapas.
    if req.llm_backend and req.llm_backend != "ollama":
    ...
    try:
        ...
        if llm.available():
            ...
            set_progress("analyzing", detail=f"Copymerda ({backend_label}) gerando prompts UGC...")
            segments = await asyncio.get_event_loop().run_in_executor(
                None, lambda: analyze_and_generate_prompts(segments, context)
            )
            lettering_count = sum(1 for s in segments if s.get("lettering"))
            set_progress("analyzing", detail=f"{lettering_count} lettering | prompts UGC prontos")
        else:
        ...
        vertical = detect_vertical(context, _compliance_rules())
        ...
        try:
            ...
            if mode == "tags" and broll_classifier.available() and \
               len(tagged) >= max(3, int(0.2 * len(brolls))):
                ...
                rerank_fn = None
                if req.vision_verify and broll_vision_verify.available():
                    ...
                    def rerank_fn(seg, q, c):
                        ...
                        set_progress("matching", current=_vis_done["n"], total=_vis_total,
                                     detail=f"IA de visão (só trechos de risco) — "
                                            f"{_vis_done['n']} verificado(s)...")
                        return broll_vision_verify.rerank(seg.get("text", ""), q, c)
                    set_progress("matching", detail="Busca semântica + visão seletiva nos trechos de risco...")
                else:
                    set_progress("matching", detail="Busca semântica por embeddings visuais...")
                _exclude = set(req.timeline_broll_paths or ())
        ...
        rhythm_counts = apply_rhythm(segments, matches)

        # 4c. Mapa do PHOENIX (se o editor clicou "Usar como base") — advisory:
...
async def tag_assets(req: TagAssetsRequest):
    ...
    Sem 'folder' usa a pasta de B-rolls salva no config."""
    folder = req.folder or _load_config().get("broll_folder", "")
    if not folder or not os.path.isdir(folder):
        raise HTTPException(400, f"Pasta inválida: {folder}")
    if not broll_classifier.available():
        raise HTTPException(400, "Tagging precisa de ANTHROPIC_API_KEY ou GEMINI_API_KEY.")

    def cb(cur, total, name):
        set_progress("tagging", current=cur, total=total, detail=name)

    set_progress("tagging", detail="Iniciando auto-tagging...")
    counts = await asyncio.get_event_loop().run_in_executor(
        None, lambda: asset_tagger.tag_folder(folder, force=req.force, progress_cb=cb)
    )
    set_progress("done")
